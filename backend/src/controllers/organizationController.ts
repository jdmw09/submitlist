...
export const createTaskGroup = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const groupResult = await query(
      `INSERT INTO task_groups (organization_id, name, description, created_by_id)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
    ...
    if (memberIds && Array.isArray(memberIds) && memberIds.length > 0) {
      for (const memberId of memberIds) {
        ...
        if (orgMemberCheck.rows.length > 0) {
          await query(
            'INSERT INTO task_group_members (group_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING',
            [group.id, memberId]
          );
        }
      }
 ...
 * Get all task groups in an organization
 */
export const getTaskGroups = async (req: AuthRequest, res: Response) => {
  try {
    const { organizationId } = req.params;
    const userId = req.user!.id;
    ...
    const result = await query(
      `SELECT tg.*,
              (SELECT COUNT(*) FROM task_group_members WHERE group_id = tg.id) as member_count,
              u.name as created_by_name
       FROM task_groups tg
       LEFT JOIN users u ON tg.created_by_id = u.id
 ...
 * Get task group details with members
 */
export const getTaskGroupDetails = async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.error('Get task group details error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
};
 ...
 * Update task group
 */
export const updateTaskGroup = async (req: AuthRequest, res: Response) => {
  try {
    const { groupId } = req.params;
    ...
    await query(
      `UPDATE task_groups
       ...
       WHERE id = $3`,
      [name || group.name, description !== undefined ? description : group.description, groupId]
    );

    res.json({ message: 'Task group updated successfully' });
  } catch (error) {
...
export const deleteTaskGroup = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Delete group (CASCADE will delete group members)
...
export const addGroupMembers = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (memberCheck.rows.length === 0 || memberCheck.rows[0].role !== 'admin') {
      return res.status(403).json({ error: 'Only organization admins can add group members' });
    }
    ...
    for (const memberId of userIds) {
      ...
      const orgMemberCheck = await query(
        'SELECT id FROM organization_members WHERE organization_id = $1 AND user_id = $2',
        [group.organization_id, memberId]
      );

      if (orgMemberCheck.rows.length > 0) {
        try {
          const result = await query(
            'INSERT INTO task_group_members (group_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING RETURNING *',
            [groupId, memberId]
          );
          if (result.rows.length > 0) {
...
export const removeGroupMember = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (memberCheck.rows.length === 0 || memberCheck.rows[0].role !== 'admin') {
      return res.status(403).json({ error: 'Only organization admins can remove group members' });
    }

    // Remove member
    await query(
      'DELETE FROM task_group_members WHERE group_id = $1 AND user_id = $2',
      [groupId, memberId]
    );
